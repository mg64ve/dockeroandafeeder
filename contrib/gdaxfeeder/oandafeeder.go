...
func (oa *OandaFetcher) Run() {
	...
	client := goanda.NewConnection(accountID, key, false)
	timeStart := time.Time{}
	...
	for {
		...
		for _, symbol := range symbols {
		granularity:
			int(oa.baseTimeframe.Duration.Seconds())
			fmt.Printf("Requesting %s %v - %v\n", symbol, timeStart, timeEnd)
			rates, err := client.GetCandlesWithinTime(symbol, timeStart, timeEnd, granularity)
			if err != nil {
...
func main() {
	...
	client := goa.NewClient("", "", "")
	params := goa.GetHistoricRatesParams{
		Start:       time.Date(2017, 12, 1, 0, 0, 0, 0, time.UTC),
		End:         time.Date(2017, 12, 1, 1, 0, 0, 0, time.UTC),
		Granularity: 60,
	}
	res, err := client.GetHistoricRates("EUR_USD", params)
	fmt.Println(res, err)
}
